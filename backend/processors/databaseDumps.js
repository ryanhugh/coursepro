...
DatabaseDumps.prototype.go = function(query, callback) {
	...
	q.defer(function(callback) {
		classesDB.find(searchQuery, {skipValidation: true},function(err, results) {
			if (err) {
				return callback(err)
			}
	...
	q.defer(function(callback) {
		sectionsDB.find(searchQuery, {skipValidation: true},function(err, results) {
			if (err) {
				return callback(err)
			}
...
if (require.main === module) {
	module.exports.go({host:'neu.edu',termId:"201710"},function (err, results) {
